...
def process_single_image(image_path, predictor, metadata, H, output_folder):
    ...
    design_map = cv2.imread(
        f"demo/{BLAST_NAME}_output/map_align_output/images/{BLAST_NAME}_drill_holes_map.png",
...
def process_folder(
    ...
    output_folder = f"demo/{BLAST_NAME}_output"
...
if __name__ == "__main__":
    ...
    input_folder = f"demo/{BLAST_NAME}_output/{BLAST_NAME}_frames"

    # generated by map_align.ipynb
    H = np.load(
        f"demo/{BLAST_NAME}_output/map_align_output/csv/{BLAST_NAME}_homography.npy"
    )
    transformed_coords_df = pd.read_csv(
        f"demo/{BLAST_NAME}_output/map_align_output/csv/{BLAST_NAME}_transformed_coordinates.csv"
    )
    ...
    # output folders
    output_frame_folder = f"demo/{BLAST_NAME}_output/frames_cv_output"
    drill_hole_folder = f"demo/{BLAST_NAME}_output/frames_with_blasting_points"
    combined_output_folder = f"demo/{BLAST_NAME}_output/frames_combined"
